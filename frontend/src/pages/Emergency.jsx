...
export default function Emergency() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 relative font-sans overflow-hidden">
      ...
      <div className="w-full max-w-lg z-10">
        <button onClick={() => navigate('/dashboard')} className="flex items-center gap-2 text-slate-400 hover:text-white transition mb-10 w-fit">
          <ArrowLeft size={20} /> Back to Dashboard
        </button>

        <div className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-black text-white tracking-tighter mb-4">Emergency <span className="text-rose-500">SOS</span></h1>
          ...
          {!activeSOS ? (
            <motion.div key="sos-button" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }} className="flex justify-center mb-12">
              <button onClick={activateSOS}
                ...
                <div className="absolute inset-0 bg-rose-600 rounded-full blur-2xl opacity-70 group-hover:opacity-100 transition animate-pulse" />
            ...
            <motion.div key="sos-active" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center mb-12">
              {loading ? (
        ...
        <div className="grid grid-cols-3 gap-4">
           {[
             ...
             { icon: Flame,      label: 'Fire Alert',color: 'amber' }
              ...
              <button key={a.label} className={`flex flex-col items-center justify-center p-4 rounded-2xl bg-slate-900/80 border border-slate-800 hover:border-${a.color}-500/50 hover:bg-slate-800 transition`}>
                 <a.icon size={28} className={`text-${a.color}-500 mb-3`} />
